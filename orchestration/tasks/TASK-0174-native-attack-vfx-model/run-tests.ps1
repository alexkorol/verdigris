...
if ($LASTEXITCODE -ne 0) { throw "compiling attack vfx tests failed with exit code $LASTEXITCODE" }

$linkCommand = $pathSetup + 'call "' + $vcvars + '" && cl /nologo "' + $testObject + '" /Fe"' + $testExe + '"'
& cmd.exe /d /s /c $linkCommand
if ($LASTEXITCODE -ne 0) { throw "linking attack vfx tests failed with exit code $LASTEXITCODE" }

& $testExe
if ($LASTEXITCODE -ne 0) { throw "attack vfx tests failed with exit code $LASTEXITCODE" }
