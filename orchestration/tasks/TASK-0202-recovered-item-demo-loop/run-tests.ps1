# TASK-0202 model-slice acceptance harness.
param()

$ErrorActionPreference = "Stop"
$taskDir = $PSScriptRoot
$root = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $taskDir))
$includeDir = Join-Path $root "native\include"
...
$testSource = Join-Path $taskDir "relic_provenance_tests.cpp"
$testObject = Join-Path $buildDir "relic_provenance_tests.obj"
$testExe = Join-Path $buildDir "relic_provenance_tests.exe"

$compile = 'call "' + $vcvars + '" && cl /nologo /std:c++20 /EHsc /W4 /I"' + $includeDir + '" /c "' + $testSource + '" /Fo"' + $testObject + '"'
...
Write-Host "TASK-0202 relic provenance model slice: PASS"
