...
const gameShots = async (browser, { night = false } = {}) => {
  ...
  if (night) {
    ...
    // Keep RAF timestamps in this same clock domain. The old harness replaced
    // performance.now() but left native RAF timestamps untouched, so the orb
    // renderer could see a large negative first dt and feed NaN to WebGL.
    await context.addInitScript(() => {
      const original = performance.now.bind(performance);
      const offset = 0.80 * 300 * 1000;
      const clockNow = () => original() + offset;
