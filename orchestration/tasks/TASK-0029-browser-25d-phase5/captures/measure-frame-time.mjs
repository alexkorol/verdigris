// Phase-5 frame-time evidence harness.
//
// Usage (with the development server already listening on :6500):
//   node measure-frame-time.mjs before
//   node measure-frame-time.mjs after
//
// The page wrapper records the synchronous duration of every game RAF
// callback after guest onboarding. The same 1440x1000 ARPG viewport and the
// same 12-second WASD workload are used for both runs. A synchronous callback
// duration measures the engine update + rendering work without counting idle
// time between frames; mean and p95 are reported over at least ten seconds.
