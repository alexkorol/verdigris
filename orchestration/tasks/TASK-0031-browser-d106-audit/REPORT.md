...
wear entries failing the notable predicate are not recoverable. The SQLite
death seam commits filtered notable relics at the lethal hit through
`entombFallenScion` ([`server/core/services/chronicles.js:206-218`](../../../server/core/services/chronicles.js#L206-L218), [`server/core/repositories/chronicles-repository.js:467-505`](../../../server/core/repositories/chronicles-repository.js#L467-L505)). The later return handler's `removeItemIdentity` is a separate JSON-Store path: it is guarded by `record.exists` and only runs when that JSON `entomb` returns a relic ([`server/player/handlers/socket-events/index.js:462-482`](../../../server/player/handlers/socket-events/index.js#L462-L482)). It is therefore incorrect to say the selected relic is universally removed only on return. In either path the dead player is then removed from the world and a successor is created from the fresh Scion profile ([`server/player/handlers/socket-events/index.js:484-507`](../../../server/player/handlers/socket-events/index.js#L484-L507), [`server/core/entities/player/fresh-scion-profile.js:21-46`](../../../server/core/entities/player/fresh-scion-profile.js#L21-L46)). Thus remaining carried state disappears with the dead in-memory player. D-106 requires every carried item and trophy to remain recoverable, so the hard path is a direct product violation.
...
[`TASK-0018 REPORT`](../TASK-0018-death-recoverability/REPORT.md#L14-L21)
and [`TASK-0025 REPORT`](../TASK-0025-instance-lifecycle-correctness/REPORT.md#L15-L30),
plus binding [`D-106`](../../DECISIONS.md#L42-L47) and
[`D-109`](../../DECISIONS.md#L61-L65) in `orchestration/DECISIONS.md`.
Together those anchors require every carried value to remain recoverable,
items/trophies to enter ordered `relic_candidates`/`lost_trophies` pools,
deterministic delayed recovery with empty successors, extraction as the only
durable-store path, and retirement that requeues an unclaimed surfaced
candidate once while ordinary floor leftovers are lost
([`TASK-0018 REPORT`](../TASK-0018-death-recoverability/REPORT.md#L41-L58),
[`TASK-0025 REPORT`](../TASK-0025-instance-lifecycle-correctness/REPORT.md#L27-L30)).
...
* SQLite relic re-entry is not native-style FIFO pool resurface. `drawEligibleRelic`
  ...
  separate 12% circulation chance ([`server/core/repositories/chronicles-repository.js:508-541`](../../../server/core/repositories/chronicles-repository.js#L508-L541), [`server/core/combat/loot.js:13-23`](../../../server/core/combat/loot.js#L13-L23)). The separate elite-only JSON-store branch is explicit at [`server/core/combat/loot.js:170-183`](../../../server/core/combat/loot.js#L170-L183): `chroniclesStore.beginRelicDrop` consumes `queued` and emits that item, with its JSON status transition defined at [`server/core/services/chronicles-store.js:490-520`](../../../server/core/services/chronicles-store.js#L490-L520). The general SQLite draw is at [`server/core/combat/loot.js:209-223`](../../../server/core/combat/loot.js#L209-L223).
* SQLite pickup claims synchronously inside `commitGroundItemPickup`: the item
  is added/removed from the scene, then `claimCirculatingRelic` calls the
  repository update from `dropped` to `claimed` ([`server/core/items/pickup.js:24-55`](../../../server/core/items/pickup.js#L24-L55), [`server/core/repositories/chronicles-repository.js:583-590`](../../../server/core/repositories/chronicles-repository.js#L583-L590)). This is distinct from the JSON Chronicle path: its action handler saves first and then calls `chroniclesStore.recoverRelic`, changing `circulating` to `recovered` ([`server/player/handlers/actions/index.js:1234-1252`](../../../server/player/handlers/actions/index.js#L1234-L1252), [`server/core/services/chronicles-store.js:523-565`](../../../server/core/services/chronicles-store.js#L523-L565)).
...
|---|---|---|---|
| Death scope | Every equipped item, pack item, and carried trophy becomes recoverable; items go to `House::relic_candidates`, trophies to `House::lost_trophies`; successor starts empty ([`D-106`](../../DECISIONS.md#L42-L47), [`TASK-0018 REPORT`](../TASK-0018-death-recoverability/REPORT.md#L14-L21)). | Soft mode keeps all values immediately through respawn; hard mode filters through `collectNotableGear` and `selectScionRelic` ([`server/core/services/chronicles.js:60-89`](../../../server/core/services/chronicles.js#L60-L89), [`server/core/services/scion-relics.js:43-70`](../../../server/core/services/scion-relics.js#L43-L70)). | Hard death destroys/losses ordinary carried values and has no trophy pool. Implement one authoritative death transfer before removing the player. |
| Relic resurface | Native recovery candidates resurface oldest-first on deterministic seeded 1-in-4 cadence ([`TASK-0018 REPORT`](../TASK-0018-death-recoverability/REPORT.md#L18-L21), [`TASK-0018 REPORT`](../TASK-0018-death-recoverability/REPORT.md#L63-L64)); recovered floor candidates requeue once at retirement ([`TASK-0025 REPORT`](../TASK-0025-instance-lifecycle-correctness/REPORT.md#L27-L30)). | Browser SQLite rows wait `run_count + 3`, then `drawEligibleRelic` oldest-first behind kill circulation; its lifecycle includes `circulating` at entomb, `dropped` on draw, and `claimed` on synchronous pickup ([`server/core/repositories/chronicles-repository.js:467-505`](../../../server/core/repositories/chronicles-repository.js#L467-L505), [`server/core/repositories/chronicles-repository.js:508-541`](../../../server/core/repositories/chronicles-repository.js#L508-L541), [`server/core/repositories/chronicles-repository.js:583-590`](../../../server/core/repositories/chronicles-repository.js#L583-L590)). The JSON store separately transitions `queued→circulating→recovered` ([`server/core/services/chronicles-store.js:447-466`](../../../server/core/services/chronicles-store.js#L447-L466), [`server/core/services/chronicles-store.js:490-520`](../../../server/core/services/chronicles-store.js#L490-L520), [`server/core/services/chronicles-store.js:523-565`](../../../server/core/services/chronicles-store.js#L523-L565)). | Cadence, storage, and event semantics differ; choose the product-authoritative pool and migrate both stores without duplicate UUIDs. |
| Trophies | Native has a separate ordered `lost_trophies` pool and dedicated resurface event ([`TASK-0018 REPORT`](../TASK-0018-death-recoverability/REPORT.md#L18-L26)). | Browser has no Player-level trophy vector; Vesselforge fragments are engine-local and socketed trophies are nested in items ([`server/core/items/vesselforge/engine.js:443-490`](../../../server/core/items/vesselforge/engine.js#L443-L490)). | Add a durable trophy representation and death/re-entry/claim paths; explicitly decide whether existing socketed trophies remain with their recovered item. |
| Instance retirement boundary | TASK-0025 retires ground state, rejects stale pickups, requeues an unclaimed surfaced relic/trophy exactly once, and loses ordinary leftovers ([`TASK-0025 REPORT`](../TASK-0025-instance-lifecycle-correctness/REPORT.md#L15-L30), [`TASK-0025 REPORT`](../TASK-0025-instance-lifecycle-correctness/REPORT.md#L56-L57)). | `destroyInstance` drops scene arrays wholesale; only restart resets SQLite `dropped` rows ([`server/core/world.js:532-545`](../../../server/core/world.js#L532-L545), [`server/core/repositories/chronicles-repository.js:87-90`](../../../server/core/repositories/chronicles-repository.js#L87-L90)). | Add an explicit browser retirement routine with active-instance membership and candidate requeue before destruction. |
| Logout/disconnect | D-109 requires no item/progress loss and a safe pull-out, never a death ([`D-109`](../../DECISIONS.md#L61-L65)). | Successful close saves before removal, but failed saves are swallowed and removal continues ([`server/Delaford.js:201-240`](../../../server/Delaford.js#L201-L240)). | Preserve current ordering, but make failure a retry/blocking boundary or durable queue; test disconnect during combat and instance teardown. |
| Chronicle ownership | Native House owns recovery pools; successor does not inherit the dead Scion's full inventory ([`D-004`](../../DECISIONS.md#L21-L23), [`TASK-0018 REPORT`](../TASK-0018-death-recoverability/REPORT.md#L14-L21)). | Browser has JSON `chroniclesStore` and SQLite `chroniclesRepository` paths in the same handler/loot surface ([`server/player/handlers/socket-events/index.js:5-30`](../../../server/player/handlers/socket-events/index.js#L5-L30), [`server/core/combat/loot.js:1-5`](../../../server/core/combat/loot.js#L1-L5)). | Migration must converge authority or define an adapter; otherwise a relic can be committed in one store and surfaced/claimed in another. |
...
1. **Authoritative death transfer:** extend the server Chronicle/relic service
   ...
   `wear` value and every `inventory.slots` value exactly once before the
   player is removed. An embedded trophy is part of its parent item: preserve
   the parent UUID plus the nested trophy IDs/payload unchanged, assign one
   recovery-owner record, and make death retry/instance retirement idempotent
   so the same parent (and therefore its trophies) cannot be registered twice.
   Standalone Vesselforge fragments currently live in `character.fragments`
   ([`server/core/items/vesselforge/engine.js:443-452`](../../../server/core/items/vesselforge/engine.js#L443-L452), [`server/core/items/vesselforge/engine.js:478-490`](../../../server/core/items/vesselforge/engine.js#L478-L490)); the forecast is a House-owned lost-trophy record keyed by a stable fragment ID/type and quantity, transferred once, removed atomically on claim, and requeued at most once on retirement. Keep successor construction in `fresh-scion-profile.js` empty except for the intentional starter contract.
...
* **Legacy data:** guest JSON snapshots have no recovery pools, and existing
  ...
  `recovered` ([`server/core/services/chronicles-store.js:447-466`](../../../server/core/services/chronicles-store.js#L447-L466), [`server/core/services/chronicles-store.js:490-520`](../../../server/core/services/chronicles-store.js#L490-L520), [`server/core/services/chronicles-store.js:523-565`](../../../server/core/services/chronicles-store.js#L523-L565), [`server/core/repositories/chronicles-repository.js:467-541`](../../../server/core/repositories/chronicles-repository.js#L467-L541)). Migration must preserve UUIDs and provenance; startup currently changes SQLite `dropped` rows only as a crash repair ([`server/core/repositories/chronicles-repository.js:87-90`](../../../server/core/repositories/chronicles-repository.js#L87-L90)).
...
* **Trophy product question:** should a trophy socketed into an item remain
  ...
  Vesselforge fragments be represented in the House pool? The constitution's
  item/history guidance explicitly covers item identity and possible death
  return ([`docs/product/VERDIGRIS_CONSTITUTION.md:47-58`](../../../docs/product/VERDIGRIS_CONSTITUTION.md#L47-L58)); D-106 requires all carried trophies to return to a recoverable pool ([`orchestration/DECISIONS.md:42-47`](../../../orchestration/DECISIONS.md#L42-L47)), and TASK-0018 implements a separate trophy pool ([`TASK-0018 REPORT`](../TASK-0018-death-recoverability/REPORT.md#L14-L26)). Those sources do not settle this browser data-shape detail.
* **Cadence product question:** D-106/TASK-0018 names the native deterministic
  1-in-4 cadence ([`orchestration/DECISIONS.md:42-47`](../../../orchestration/DECISIONS.md#L42-L47), [`TASK-0018 REPORT`](../TASK-0018-death-recoverability/REPORT.md#L18-L21), [`TASK-0018 REPORT`](../TASK-0018-death-recoverability/REPORT.md#L63-L64)), while the browser currently uses run-delay and kill chance ([`server/core/repositories/chronicles-repository.js:467-541`](../../../server/core/repositories/chronicles-repository.js#L467-L541), [`server/core/combat/loot.js:170-183`](../../../server/core/combat/loot.js#L170-L183)). Owner/architect confirmation is needed before changing live Chronicle timing, especially for already-circulating relics.
...
Captured read-only proof distinguishes the coordinator checkout from this
isolated worker worktree. The coordinator checkout was already dirty before
this audit and those paths were not touched:

```text
PS C:\Users\Alex\Documents\ChatGPT\verdigris> git status --short
 M docs/loop-journal.md
?? .ci-artifacts/
?? .codex/
?? .repo-backups/
?? .test-state/
PS ...> git status -sb
## codex/native-reconstitution...origin/codex/native-reconstitution [ahead 4]
 M docs/loop-journal.md
?? .ci-artifacts/
?? .codex/
?? .repo-backups/
...
The worker preflight was clean at base `8e4a977c`; `git fetch --prune origin`
completed, `origin` was `https://github.com/alexkorol/verdigris.git`, and
`git rev-list --left-right --count 'HEAD...@{upstream}'` reported no upstream
configured for this worker branch. After the report revision is committed,
the worker `git status --short` is expected to be empty; the coordinator dirt
above remains pre-existing and is outside this task's owned paths.
