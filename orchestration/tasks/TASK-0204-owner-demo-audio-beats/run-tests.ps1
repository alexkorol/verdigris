# TASK-0204 model-slice acceptance harness.
param()

$ErrorActionPreference = "Stop"
$taskDir = $PSScriptRoot
$root = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $taskDir))
$audioInclude = Join-Path $root "native\audio"
...
$testSource = Join-Path $taskDir "owner_demo_audio_beats_tests.cpp"
$testObject = Join-Path $buildDir "owner_demo_audio_beats_tests.obj"
$testExe = Join-Path $buildDir "owner_demo_audio_beats_tests.exe"

$includeFlags = '/I"' + $audioInclude + '" /I"' + $clientInclude + '"'
...
Write-Host "TASK-0204 owner demo audio beats model slice: PASS"
