# TASK-0158 acceptance harness: compiles the production pane model header
# (native/client/pane_model.hpp) together with this task's self-contained
# test source using the installed MSVC environment, runs the tests, and
# leaves every output artifact under this task folder only.
param()
...
$testObject = Join-Path $buildDir "pane_model_tests.obj"
$testExe = Join-Path $buildDir "pane_model_tests.exe"
...
if ($LASTEXITCODE -ne 0) { throw "compiling the pane model tests failed with exit code $LASTEXITCODE" }

$linkCommand = $pathSetup + 'call "' + $vcvars + '" && cl /nologo "' + $testObject + '" /Fe"' + $testExe + '"'
& cmd.exe /d /s /c $linkCommand
if ($LASTEXITCODE -ne 0) { throw "linking the pane model tests failed with exit code $LASTEXITCODE" }

& $testExe
if ($LASTEXITCODE -ne 0) { throw "pane model tests failed with exit code $LASTEXITCODE" }
Write-Host "TASK-0158 pane model acceptance harness: PASS"
