...
bool gateb_hunt_until_relic_surfaces(LoopbackClient& client,
  ...
  // GATEB-FLAKE patch: a painted ranged-volley circle is the same public
  // warning surface as the boss slam - every client receives the
  // monster:telegraph payload - so a normal player sidesteps it. Track the
  // few most recent circles so an overlapping slinger pair cannot pin the
  // recovery leg: the old driver only dodged boss:ground-slam, and two
  // staggered ranged:volley casters beyond melee reach land a hit about every
  // 1.3 s, which never lets the two-second quiet window open after the relic
  // surfaces (relic_surfaced also blocks the low-health retreat), so the
  // successor was ground down while its swings could not reach the casters.
  struct GatebDangerCircle {
    int x, y, radius;
    std::chrono::steady_clock::time_point clear_at;
  };
  std::vector<GatebDangerCircle> volley_circles;
  ...
  while (now() < deadline) {
    ...
    while (client.scan_from(index, [](const LoopbackClient::Line&) { return true; }, &line)) {
      ...
      if (line.env.event == "monster:telegraph" &&
          gateb_str(line.env.data, "skillId") == "ranged:volley") {
        // GATEB-FLAKE patch: record the painted volley circle with the same
        // clock-skew buffer convention as the slam, capped so a long hunt
        // cannot grow the list without bound.
        GatebDangerCircle circle;
        circle.x = static_cast<int>(gateb_num(line.env.data, "x"));
        circle.y = static_cast<int>(gateb_num(line.env.data, "y"));
        circle.radius = static_cast<int>(gateb_num(line.env.data, "radius", 1));
        circle.clear_at =
            line.at + std::chrono::milliseconds(static_cast<int>(
                          gateb_num(line.env.data, "durationMs", 800)) + 120);
        volley_circles.push_back(circle);
        if (volley_circles.size() > 6)
          volley_circles.erase(volley_circles.begin());
      }
    ...
    bool acted = false;
    // GATEB-FLAKE patch: dodge every active painted circle, not only the
    // boss slam. Expired volleys drop out; the slam joins the set while its
    // own window runs. The step scores the WORST remaining clearance across
    // all active circles so overlapping volleys from two casters are escaped
    // with one deterministic choice (lowest heading breaks ties), and steps
    // that would land on a wall or stair tile are skipped so the sidestep
    // never pins against the warren ribs.
    volley_circles.erase(
        std::remove_if(volley_circles.begin(), volley_circles.end(),
                       [&](const GatebDangerCircle& circle) {
                         return circle.clear_at <= now();
                       }),
        volley_circles.end());
    std::vector<GatebDangerCircle> danger = volley_circles;
    if (now() < slam_clear_at)
      danger.push_back({slam_x, slam_y, slam_radius, slam_clear_at});
    bool inside_circle = false;
    for (const auto& circle : danger)
      if (chebyshev(tile_x, tile_y, circle.x, circle.y) <= circle.radius)
        inside_circle = true;
    if (inside_circle) {
      // A normal player steps out of the marked circle before it resolves.
      const auto dodgable = [&](int x, int y) {
        if (x <= 0 || x >= 39 || y <= 0 || y >= 39) return false;
        if (gateb_on_stairs(view, x, y)) return false;
        const bool rib = x == 12 || x == 18 || x == 24 || x == 30;
        const bool gap = (y >= 10 && y <= 12) || (y >= 19 && y <= 21) ||
                         (y >= 26 && y <= 28);
        return !rib || y < 3 || y >= 37 || gap;
      };
      int best_heading = -1;
      ...
      for (int candidate = 0; candidate < 4; ++candidate) {
        ...
        if (!dodgable(nx, ny)) continue;
        int score = 1000000;
        for (const auto& circle : danger)
          score = (std::min)(score,
                             chebyshev(nx, ny, circle.x, circle.y) -
                                 circle.radius);
        if (score > best_score) { best_score = score; best_heading = candidate; }
      }
      if (best_heading >= 0) gateb_step(client, best_heading);
      // Keep trading blows while sidestepping: a sidestep and a swing are
      // both ordinary inputs. Starving the swing during the dodge let
      // adjacent adds chew through the heir unpunished.
      const auto since_incoming_dodge = last_incoming == never
                                            ? std::chrono::hours(24)
                                            : now() - last_incoming;
      const auto since_outgoing_dodge = last_outgoing == never
                                            ? std::chrono::hours(24)
                                            : now() - last_outgoing;
      if (best_heading < 0 || since_incoming_dodge < std::chrono::seconds(2) ||
          since_outgoing_dodge < std::chrono::seconds(1)) {
        gateb_swing(client, fight_heading);  // cornered or engaged: keep fighting
      }
      acted = true;
    }
    if (!acted) {
