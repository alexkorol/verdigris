...
| VG-ACT-005 | new | catalog-typed warning window (`telegraph-spec`) plus dodge-clear (`telegraph-dodge`); owner strip paints ticks+footprint and Dodge clear / Life holds; Dodge clear stays off WASD/objective/Tin village/Life; ms/50 and ghost hit cannot invent damage. Core ACT stays Kimi |
...
strip paints Dodge clear / Life holds and parks off WASD, the objective,
Tin village, and Life. Covering those combat surfaces cannot certify.
Ghost hit cannot certify an avoided sweep. Scenarios `telegraph-spec` /
`telegraph-dodge`. Evidence `docs/execution/evidence/VG-ACT-005.json`.
