...
- **Framekit 2.0 raster UI** — the complete 99-image flagship game pack is
  ...
  in production. The House/Scion front door also owns WIZARD's laurel crest,
  lion knocker, sealed-lineage keyhole, and naming field; the vector kit is an
  explicit failure fallback rather than the normal visual path.
...
fallen-heirloom circulation are now playable end to end. Either an elite or a
named Warden can return a circulating relic, so the recovery promise cannot be
stranded by the Warden's stronger `boss` rarity. Twenty-three real
