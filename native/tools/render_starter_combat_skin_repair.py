...
args=sys.argv[sys.argv.index('--')+1:];sex=args[0];clip=args[1] if len(args)>1 else 'attack';count=16 if clip=='attack' else 8
source=OUT/'transition-v2'/sex/'club-attack-transition.blend' if clip=='attack' else OUT/'blender'/f'{sex}-{clip}.blend';dest=OUT/'transition-v3-skin'/(sex if clip=='attack' else f'{sex}-{clip}')
