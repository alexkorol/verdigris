...
`ProtocolSession` owns the progression, admission, rewards and persistence. `WorldSimulation` supplies the village collision map and actual combat actors. The client submits typed intents and draws the replicated objective. Scenery, paths, collision, interaction locations and the normal scene transition all use the same village coordinates. `include/verdigris/starter_layout.hpp` generates a bounded, seeded layout shared by authority and presentation: clustered trees and shrubs, scattered rocks and grass, and paths with reserved combat and interaction clearings. Tree trunks and rocks block their occupied tiles; the former house footprints are open. The layout is cached and does not change on reconnect or camera movement. The scene has no old portal or generic stone collision-wall rendering.
...
`assets/first-slice/village-v3` retains the Blender-guided ImageGen props and measured fixed-grid transfer provenance. The whole-building sprites are no longer placed in the starter map. The well, palisade sections and trees remain small scenery elements, joined by the existing shrub, rock and grass cutouts at the same 48 pixels/metre. The first failed paint remains a rejected source, never installed. Source paint retains small silhouette deviations recorded in its review. The village uses fixed daylight and no depth-of-field blur so the pixel result is inspectable.
