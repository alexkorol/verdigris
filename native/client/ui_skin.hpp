...
// shell wrapped around the WIZARD plates. Mirrors the web client's design
// tokens (src/assets/scss/abstracts/_tokens.scss) so both clients read as
// one product.
...
// (merge) cache kind 42: kind 41 belongs to cooldown_wedge; sharing it would
// alias the two cached layers whenever w/h/bucket coincide.
inline void xp_meter(HDC dc, const RECT& rect, double fraction) {
  ...
  const CachedLayer* layer =
      cached_layer(layer_key(42, w, h, kGold, bucket), w, h, paint_meter);
  if (layer) {
    blend_layer(dc, *layer, rect.left, rect.top);
    return;
  }
