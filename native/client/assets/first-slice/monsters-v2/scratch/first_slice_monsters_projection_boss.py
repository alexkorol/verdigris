...
OUT=ROOT/'projection-proof/boss-v3-faces'
...
dx=dy=0;records={};refine=True;sex='well-alpha';gait='death'
...
target=ROOT/'death-v2/well-alpha-death-v2.blend'
...
paint.pack();bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'blender/well-alpha-death-projection.blend'),compress=True)
(OUT/'reports/well-alpha-death-projection.json').write_text(json.dumps(report,indent=2))
...
for direction,angle in [('front',0),('right',90),('back',180),('left',270)]:
 ...
 for phase in range(4):
  s.frame_set(phase);s.render.filepath=str(OUT/f'frames/well-alpha-death-{direction}-{phase:02d}.png');bpy.ops.render.render(write_still=True)
