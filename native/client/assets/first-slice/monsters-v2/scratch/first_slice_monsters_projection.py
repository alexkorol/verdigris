...
OUT=ROOT/'projection-proof'
nearest_surface='--nearest-surface' in sys.argv
if nearest_surface:OUT=OUT/'v2'
for folder in ['frames','blender','reports']:(OUT/folder).mkdir(parents=True,exist_ok=True)
source=ROOT/'projection-proof/appearance-atlas.png';idle=ROOT/'accepted/blender/pack-wolf-walking.blend'
dx=dy=0;records={};refine=True;sex='pack-wolf';gait='death'
...
target=ROOT/'death-v2/pack-wolf-death-v2.blend'
...
s.render.film_transparent=True;s.render.image_settings.color_mode='RGBA';s.render.resolution_x=s.render.resolution_y=96;s.render.resolution_percentage=100
...
report['frame']=[96,96];report['anchor']=[48,76];report['pixels_per_metre']=48
paint.pack();bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'blender/pack-wolf-death-projection.blend'),compress=True)
(OUT/'reports/pack-wolf-death-projection.json').write_text(json.dumps(report,indent=2))
...
for direction,angle in [('front',0),('right',90),('back',180),('left',270)]:
 ...
 for phase in range(4):
  s.frame_set(phase);s.render.filepath=str(OUT/f'frames/pack-wolf-death-{direction}-{phase:02d}.png');bpy.ops.render.render(write_still=True)
