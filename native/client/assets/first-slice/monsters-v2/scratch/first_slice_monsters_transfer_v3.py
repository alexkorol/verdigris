...
def recover(raw,clip):
    m=json.loads((ROOT/'transfer-manifest-v3.json').read_text())['clips'][clip];refs=m['frames'];w,h=m['frame']; layout=m['layout']; scale=layout['scale']; columns=layout['columns'];slot_w,slot_h=layout['slot'];inset_x,inset=layout['inset'];row_offset=layout.get('row_offset',0)
