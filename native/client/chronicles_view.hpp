...
void paint_lineage_door(ClientState& state,HDC dc,const RECT& bounds,render::List& trace) {
  ...
  const int content_top=top+105*s,footer=bottom-104*s;
  ...
  int y=content_top;
  for(std::size_t h=0;h<model.chronicle.houses.size();++h) {
    const auto& house=model.chronicle.houses[h];
    if(house.id!=state.selected_house_id) continue;
    ...
    if(model.chronicle.houses.size()>1) {
      button({"","house-select",model.chronicle.houses[(h+1)%model.chronicle.houses.size()].id,""},
          painted_house_name(house.name)+"  >",{roster_left,y,roster_right,y+40*s},true);
    }
    y+=48*s;break;
  }
  if(!has_house) {
    ...
    for(const auto& action:state.chronicles_menu)
      if(action.command=="set-out"||action.command=="select-scion") {
        for(const auto& house:model.chronicle.houses) if(house.id==state.selected_house_id)
          for(const auto& scion:house.scions) if(scion.id==action.arg) admissions.push_back(action);
      }
  ...
  button({"","back-title","",""},"Back to title",
      {left+32*s,bottom-42*s,split-20*s,bottom-8*s});
