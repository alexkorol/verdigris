...
inline bool is_projectile_warning(const PresentationEvent& event) {
  ...
  // Merge note (natrecon x aaa): an authoritative server telegraph that
  // already carries a ground position (has_position, x/y, radius) must stay
  // on the aaa Telegraph branch so its destination/radius/windup are used.
  // Only JS-parity world:projectile envelopes (text "projectile" or bare
  // "ranged:volley" without a position) take the TASK-0108 warning path.
  if (event.has_position) return false;
